...
Formula::Formula(const Formula& formula)
    : literals_not_fixed(formula.literals_not_fixed), assignments(formula.assignments) {
    for (std::unordered_set<std::shared_ptr<Clause>>::const_iterator c = formula.clauses.begin();
...
void Formula::add_clause(std::shared_ptr<Clause> clause) {
    ...
    for (std::unordered_map<std::string, bool>::const_iterator literal = literals.begin();
        ...
        if (literal_in_clauses.find(literal->first) != literal_in_clauses.end()) {
            ...
            literal_in_clauses.insert(std::make_pair(
                literal->first, std::unordered_set<std::shared_ptr<Clause>>{clause}));
        }
    }
